...
新版用户 API `POST /api/v1/extractions` 的覆盖规则：
...
请求级 Cookie、代理不会写入解析历史，也不会出现在响应中。使用任一请求级覆盖或显式 `null` 时，本次不会读取或更新持久缓存映射；作品版本和查询记录仍会保存。

旧兼容接口 `/xhs/detail` 对空字符串或 `null` 保持旧语义：视为未覆盖并继承默认值。其响应会清除 `params.cookie` 与 `params.proxy`。
...
新版用户接口没有下载参数，始终按管理端策略决定是否保存。旧 `/xhs/detail` 额外保留请求级门控：
...
- 请求级 Cookie、代理覆盖或显式 `null`：始终绕过持久缓存映射。
